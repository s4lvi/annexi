...
function findPathBetweenCities(lobby, sourceCity, targetCity, playerId) {
  ...
  class Node {
    constructor(x, y, cost = 0, parent = null) {
      ...
      this.key = `${x},${y}`; // Unique key for this node
      ...
      // Total cost is cost so far plus heuristic
  ...
  function getMovementCost(x, y) {
    ...
    // If there's a structure, calculate the cost based on ownership
    if (structure) {
      // Friendly structure - pass through with minimal cost
      ...
      switch (structure.structure.name) {
        case "Wall":
          return 10; // Expensive but not impossible
        case "Watchtower":
          return 5;
        case "Archer":
          return 3;
        default:
          return 2; // Default cost for unknown structures
      }
    }
    ...
    if (tileOwner && tileOwner !== playerId) {
      return 2; // Slightly higher cost for enemy territory
    }

    // Basic movement cost for friendly/neutral territory
    return 1;
  }

  // Get neighboring tiles considering hex grid layout
  function getNeighbors(node) {
    const { x, y } = node;
    // Use the hex adjacency calculation from your existing code
    ...
    return adjacentOffsets
      .map(([dx, dy]) => {
        ...
        // If the cost is infinite, this neighbor is not valid
        if (cost === Infinity) return null;

        return new Node(
          newX,
          newY,
          node.cost + cost, // Add the movement cost to the current cost
          node // Set the current node as the parent
        );
      })
      .filter((neighbor) => neighbor !== null); // Remove invalid neighbors
  }
  ...
  function findPath() {
    ...
    // Priority queue for open nodes (sorted by f value)
    const openSet = [startNode];
    // Set of visited nodes
    const closedSet = new Set();
    // Map of nodes by key for quick lookup
    const nodeMap = { [startNode.key]: startNode };

    while (openSet.length > 0) {
      // Sort open set by f value (lower is better)
      ...
      for (const neighbor of neighbors) {
        ...
        if (!existingNode) {
          ...
          // We found a better path to this node, update it
...
module.exports = function (io) {
  io.on("connection", (socket) => {
    ...
    socket.on("playerReady", (data) => {
      ...
      console.log("Checking readiness for step", lobby.turnStep, ":", allReady),
        " there were ",
      ...
      if (allReady) {
        ...
        if (lobby.turnStep < 6) {
          ...
          // At step 3, auto-ready players without structure cards
          if (lobby.turnStep === 3) {
            setTimeout(() => {
              let autoReadyTriggered = false;

              // Check each player for no structure cards
              lobby.players.forEach((player) => {
                ...
                if (!hasStructureCards) {
                  console.log(
                    `Auto-ready player ${player.username} (no structure cards)`
                  );
                  player.readyForStep = true;
                  ...
                  if (player.socketId) {
                    io.to(player.socketId).emit("autoReady", {
                      message:
                        "You have no structure cards. Skipping structure placement.",
                    });
                  }
                }
              });

              // If we auto-readied anyone, check if now everyone is ready
              if (
                autoReadyTriggered &&
                lobby.players.every((p) => p.readyForStep)
              ) {
                // Advance to the next step using the same logic above
                lobby.players.forEach((p) => (p.readyForStep = false));
                lobby.turnStep++;
                io.to(`lobby-${lobbyId}`).emit("stepCompleted", {
                  turnStep: lobby.turnStep,
          ...
          // At step 4, auto-ready players without unit cards
          if (lobby.turnStep === 4) {
            setTimeout(() => {
              let autoReadyTriggered = false;

              // Check each player for no unit cards
              lobby.players.forEach((player) => {
                const hasUnitCards =
                  player.inventory &&
                ...
                if (!hasUnitCards) {
                  console.log(
                    `Auto-ready player ${player.username} (no unit cards)`
                  );
                  player.readyForStep = true;
                  autoReadyTriggered = true;
                  ...
                  if (player.socketId) {
                    io.to(player.socketId).emit("autoReady", {
                      message: "You have no unit cards. Skipping army queue.",
                    });
                  }
                }
              });
          ...
          // At step 5, auto-ready players without armies
          if (lobby.turnStep === 5) {
            setTimeout(() => {
              let autoReadyTriggered = false;

              // Check each player for no army
              lobby.players.forEach((player) => {
                if (!player.queuedArmy || player.queuedArmy.length === 0) {
                  console.log(`Auto-ready player ${player.username} (no army)`);
                  player.readyForStep = true;
                  autoReadyTriggered = true;
                  ...
                  if (player.socketId) {
                    io.to(player.socketId).emit("autoReady", {
                      message:
                        "You have no army queued. Skipping target selection.",
                    });
                  }
                }
              });
          ...
          // turnStep === 6: battle phase. Emit completion and then reset for a new turn.
          io.to(`lobby-${lobbyId}`).emit("stepCompleted", {
            turnStep: lobby.turnStep,
          });
          console.log(
    ...
    socket.on("buildCity", (data) => {
      ...
      // Determine if this is the first city (Capital) or a regular city
      ...
      const newCity = {
        ...
        level: 1,
        radius: 6,
        type: cityType, // Add the type to the city object
      };
      ...
      lobby.pendingCities.push({
        ...
        playerId: _id,
        radius: 6,
        type: cityType, // Also add the type to the pending city
      });

      io.to(`lobby-${lobbyId}`).emit("buildCitySuccess", {
        username: player.username,
        type: cityType, // Send the city type in the success event
        level: newCity.level,
        x: newCity.tile.x,
        y: newCity.tile.y,
        playerId: _id,
    ...
    socket.on("selectTarget", (data) => {
      const { lobbyId, sourceCity, targetCity } = data;
      const lobby = lobbies[lobbyId];
      ...
      console.log(
        `Player ${player.username} selected target: from (${sourceCity.x},${sourceCity.y}) to (${targetCity.x},${targetCity.y})`
      );

      // Validate source city ownership and type
      const sourceCityValid = lobby.players.some(
        (p) =>
          p._id === playerId &&
          p.cities &&
          p.cities.some(
            (city) =>
              ...
              ["Capital City", "Fortified City"].includes(city.type)
          )
      );

      if (!sourceCityValid) {
        socket.emit("targetSelectionError", {
          message:
            "Invalid source city. Must be a Capital or Fortified city that you own.",
        });
        return;
      }

      // Validate target city (must be enemy city)
      const targetCityValid = lobby.players.some(
        (p) =>
          p._id !== playerId &&
          p.cities &&
          p.cities.some(
            (city) =>
              city.tile.x === targetCity.x && city.tile.y === targetCity.y
          )
      );
      ...
      if (!targetCityValid) {
        socket.emit("targetSelectionError", {
          message:
            "Invalid target city. Must be a city owned by another player.",
        });
        return;
      }

      // Calculate path using A* algorithm
      const path = findPathBetweenCities(
        lobby,
        sourceCity,
        targetCity,
        playerId
      ...
      if (!path) {
        socket.emit("targetSelectionError", {
          message: "No valid path found between cities.",
        });
        return;
      }

      // Store the path and selection in the player's data for battle resolution
      player.battlePlan = {
        sourceCity,
        targetCity,
        path,
      ...
      // Broadcast the target selection to all players
      io.to(`lobby-${lobbyId}`).emit("targetSelected", {
        ...
        targetCity,
        path,
        message: `${player.username} is targeting a city at (${targetCity.x},${targetCity.y}).`,
      });

      // Notify the player specifically with the full path details
